...
var initMagicToolboxFunction = function() {
    ...
    if (jQuery('#admin_bar_iframe').length) {
        jQuery(document.body).append('<style type="text/css">.mz-zoom-window { margin-top: -'+jQuery('#admin_bar_iframe').height()+'px; </style>');
    }

    normalizeMagicSelectors();
...
function normalizeMagicSelectors() {
    var $minWidth = $minHeight = 300;
    jQuery('.MagicToolboxSelectorsContainer a > img').each(function(){
        if (jQuery(this).is(":visible")) {
        $minHeight = Math.min($minHeight,jQuery(this).height());
        $minWidth = Math.min($minWidth,jQuery(this).width());
        }
    });
    if (true || jQuery('.MagicToolboxContainer').hasClass('thumbnails-bottom')) {
        jQuery('.MagicToolboxSelectorsContainer a > img').height($minHeight+'px').show();
    } else { 
        jQuery('.MagicToolboxSelectorsContainer a > img').width($minWidth+'px').show();
    }
}