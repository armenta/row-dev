...
var nodeListForEach = __webpack_require__(16);

// EXTERNAL MODULE: ./source/scripts/polyfills/elementClosest.js
var elementClosest = __webpack_require__(17);
...
// CONCATENATED MODULE: ./node_modules/pxs-video/dist/index.es.js
...
var PxsVideo = function () {
  function PxsVideo(section) {
    index_es_classCallCheck(this, PxsVideo);
...
/* harmony default export */ var dist_index_es = (PxsVideo);
...
/* harmony default export */ var rimg_dist_index_es = (rimg);
...
function init() {
  ...
  state.instance = rimg_dist_index_es(selector, options);
...
var EventHandler = __webpack_require__(0);
...
var fitvids = __webpack_require__(13);
...
var Rte_Rte = /*#__PURE__*/function () {
  function Rte(_ref) {
    ...
    this.el = el;
    if (rtes.has(el)) return;
    rtes.set(this.el, this);
    this.setupTabs();
    fitvids_default()({
      // pxs-video is already responsive width and fitvids adds a wrapper
      // which breaks the overlay image, among other things.
      ignore: ['.pxs-video iframe']
    });
  }
  ...
  _createClass(Rte, [{
    ...
    value: function setupTabs() {
      this.groupedContent = new dist_default.a(this.el, {
        layout: 'tabs',
        intelliparse: false
      });
    }
...
// CONCATENATED MODULE: ./source/scripts/components/Share.js
...
function openShare(e) {
  e.stopPropagation();
  var targetShareWidget = e.target.closest('.article-meta-wrap');
  var otherShareWidgets = Array.prototype.filter.call(document.querySelectorAll('.article-meta-wrap'), function (el) {
    return el !== targetShareWidget;
  });
  targetShareWidget.classList.add('active');
  otherShareWidgets.forEach(function (el) {
    return el.classList.remove('active');
  });
}

function closeShare(e) {
  if (!e.target.closest('.share-widget-toggle') && !e.target.closest('.share-widget')) {
    var articleMetaWrap = document.querySelector('.article-meta-wrap');

    if (articleMetaWrap) {
      articleMetaWrap.classList.remove('active');
    }
  }
}

var Share_Share = /*#__PURE__*/function () {
  function Share(_ref) {
    var _this = this;
    ...
    this.el = el;
    this.shareWidgetToggles = this.el.querySelectorAll('.share-widget-toggle');

    this.onBodyClick = function (e) {
      return closeShare(e);
    };

    this.onShareClick = function (e) {
      return openShare(e);
    }; //Init RTEs in articles


    this.rtes = [];
    this.el.querySelectorAll('[data-rich-text-area]').forEach(function (el) {
      return _this.rtes.push(new Rte_Rte({
        el: el
      }));
    });

    if (!this.shareWidgetToggles) {
      return;
    }

    document.body.addEventListener('click', this.onBodyClick);
    this.shareWidgetToggles.forEach(function (el) {
      el.addEventListener('click', _this.onShareClick);
    });
  }
  ...
  Share_createClass(Share, [{
    ...
    value: function unload() {
      ...
      if (this.shareWidgetToggles) {
        this.shareWidgetToggles.forEach(function (el) {
          el.removeEventListener('click', _this2.onShareClick);
        });
      }
    }
  }]);
...
var just_debounce = __webpack_require__(5);
...
var Modal_unlockScrollLock = function unlockScrollLock() {
...
var Modal_Modal = /*#__PURE__*/function () {
  function Modal() {
    ...
    this.modal = null;
    this.events = new EventHandler_default.a(); // Extend default vanilla-modal callbacks back to instantiator of Modal
    ...
    this.modalInner = this.modalEl.querySelector('[data-modal-inner]');
  ...
  Modal_createClass(Modal, [{
    ...
    value: function open(selector) {
      ...
      this.events.register(window, 'keydown', this._closeEsc);
    }
  }, {
    key: "close",
    ...
    value: function position() {
      ...
      var modalHeight = this.modalInner.getBoundingClientRect().height;
      var modalPadding = parseInt(window.getComputedStyle(this.modalEl).paddingTop, 10) * 2;
      var offset = (windowHeight - modalPadding - modalHeight) / 2;
      this.modalInner.style.marginTop = offset > 0 ? "".concat(offset, "px") : 0;
    }
    /**
    ...
    value: function _addModalClass(handle) {
      this.modalEl.classList.add("modal--".concat(handle));
    }
    /**
    ...
    value: function _removeModalClass() {
      var modalClass = this.modalEl.className.match(/modal--[\w-]*\b/);

      if (!modalClass) {
        return;
      }

      this.modalEl.classList.remove(modalClass[0]);
    }
  }, {
    key: "_onClose",
    value: function _onClose() {
      ...
      this.body.classList.remove(this.visibleClass);
      this.events.unregisterAll();
      this.modalInner.style.marginTop = '';
      this.options.onClose();
      removeTrapFocus(this.modalEl);
    ...
    value: function _onOpen() {
      ...
      this.position();
      ScrollLock.lock(this.modalEl);
      this.body.classList.add(this.visibleClass);
      this.events.register(window, 'resize', just_debounce_default()(function () {
        return _this3.position();
      }, 16, true, true));
      this.options.onOpen();
      trapFocus(this.modalEl);
    }
  }, {
    key: "_onBeforeClose",
...
// CONCATENATED MODULE: ./node_modules/@pixelunion/shopify-variants/dist/shopify-variants.es.js
...
function shopify_variants_es_defineProperty(obj, key, value) {
  if (key in obj) {
    Object.defineProperty(obj, key, {
      value: value,
      enumerable: true,
...
function shopify_variants_es_ownKeys(object, enumerableOnly) {
  var keys = Object.keys(object);
...
function shopify_variants_es_objectSpread2(target) {
  for (var i = 1; i < arguments.length; i++) {
    var source = arguments[i] != null ? arguments[i] : {};

    if (i % 2) {
      shopify_variants_es_ownKeys(source, true).forEach(function (key) {
        shopify_variants_es_defineProperty(target, key, source[key]);
      });
    } else if (Object.getOwnPropertyDescriptors) {
      Object.defineProperties(target, Object.getOwnPropertyDescriptors(source));
    } else {
      shopify_variants_es_ownKeys(source).forEach(function (key) {
        Object.defineProperty(target, key, Object.getOwnPropertyDescriptor(source, key));
      });
    }
  }
...
function () {
  function Variants(product, variants, options, config) {
    ...
    var defaultConfig = {
      ...
      onSwitchVariant: function onSwitchVariant() {},
      mixedControls: false,
    ...
    this.options = options;
    this.selectOptions = Array.prototype.filter.call(this.options, function (option) {
      return option.matches(_this.optionsTypes.select);
    });
    this.radioOptions = Array.prototype.filter.call(this.options, function (option) {
      return option.matches(_this.optionsTypes.radio);
    });
    this.config = shopify_variants_es_objectSpread2({}, defaultConfig, {}, config);
  ...
  shopify_variants_es_createClass(Variants, [{
    ...
    value: function _switchVariant() {
      ...
      this.config.onSwitchVariant(data);
...
/* harmony default export */ var shopify_variants_es = (Variants);
...
var ProductDetails_ProductDetails = /*#__PURE__*/function () {
  function ProductDetails(options) {
    ...
    this.onVariantChange = options.onVariantChange || function () {};
    ...
    if (this.variants) {
      ...
      this.enableHistory = !this.isThemeEditor && history.replaceState && this.settings.enableHistory;
      this.variantHelper = new shopify_variants_es(this.product, this.variants, this.options, {
        onSwitchVariant: function onSwitchVariant(data) {
    ...
    this.modal = new Modal_Modal({
      onClose: function onClose() {
        return _this.modal.unload();
      },
      modalId: 1
    });
    this.surfacePickUp.load(selectedVariant().id);
    this.surfacePickUp.onModalRequest(function (contents) {
      var container = _this.el.querySelector('[data-surface-pick-up-modal-contents]');

      var variantTitle = _this.variantHelper ? "<div class=\"surface-pick-up-modal__variant\">".concat(selectedVariant().title, "</div>") : '';
      ...
      if (!container) {
        ...
        _this.el.querySelector('.product-details__form').appendChild(container);
      } else {
        container.innerHTML = modalContents;
      }

      _this.modal.open('[data-surface-pick-up-modal-contents]', 'surface-pick-up');
    });
  }
  ...
  ProductDetails_createClass(ProductDetails, [{
    ...
    value: function _updatePrice(variant) {
      ...
      var variantPrice = Shopify.formatMoney(variant.price, window.Theme.moneyFormat);
      var variantComparePrice = Shopify.formatMoney(variant.compare_at_price, window.Theme.moneyFormat);

      if (variant.available) {
        atcButtonText.innerText = this.settings.addToCartText;
        atcButton.disabled = false;
      } else {
        atcButtonText.innerText = this.settings.soldOutText;
      ...
      if (variant.compare_at_price > variant.price) {
        price.classList.add('product-price__minimum--sale');
        priceCompare.innerHTML = variantComparePrice;
        priceCompare.classList.remove('product-price__compare--hidden');
      } else {
        price.classList.remove('product-price__minimum--sale');
        priceCompare.innerHTML = '';
        priceCompare.classList.add('product-price__compare--hidden');
      }
    }
  }, {
    key: "_updateTaxLine",
    value: function _updateTaxLine(variant) {
      var taxLine = this.variantFields.taxLine;
      if (!taxLine) return;
    ...
    value: function _updateUnitPrice(variant) {
      var _this$variantFields2 = this.variantFields,
          unitPrice = _this$variantFields2.unitPrice,
          totalQuantity = _this$variantFields2.totalQuantity,
          unitPriceAmount = _this$variantFields2.unitPriceAmount,
          unitPriceMeasure = _this$variantFields2.unitPriceMeasure;
      ...
      if (unitPrice && variant.unit_price_measurement) {
        ...
        unitPriceAmount.innerHTML = Shopify.formatMoney(variant.unit_price, window.Theme.moneyFormat);
        ...
        unitPrice.classList.remove('product-details__unit-price--hidden');
      } else if (unitPrice) {
        unitPrice.classList.add('product-details__unit-price--hidden');
      }
    }
  }, {
    key: "_updateHistory",
    ...
    value: function _onSwitchVariant(_ref) {
...
var ImageZoom_ImageZoom = /*#__PURE__*/function () {
  function ImageZoom(el) {
    ...
    this._bindLayoutEvent = this._bindLayoutEvent.bind(this);
    this._unbindLayoutEvent = this._unbindLayoutEvent.bind(this);
    this._layoutHandler = this._layoutHandler.bind(this);
    this._toggleZoom = this._toggleZoom.bind(this);
    this._resetZoom = this._resetZoom.bind(this);
    this._positionZoom = this._positionZoom.bind(this);
  ...
  ImageZoom_createClass(ImageZoom, [{
    ...
    value: function _bindEvents() {
      ...
      this.productZoomClick = this.events.register(this.el, 'click', function (event) {
        return _this._toggleZoom(event.clientX, event.clientY);
      });
      window.addEventListener('resize', this.resetZoom);
    }
  }, {
    key: "_unbindEvents",
    value: function _unbindEvents() {
      this.events.unregister(this.productZoomClick);
      window.removeEventListener('resize', this.resetZoom);
    }
  }, {
    key: "_bindLayoutEvent",
    value: function _bindLayoutEvent() {
      this.requestAnimationFrameId = window.requestAnimationFrame(this.layoutHandler);
    }
  }, {
    ...
    value: function _layoutHandler() {
      var scale = this.img.naturalWidth / this.el.getBoundingClientRect().width;
      var zoomable = scale >= 1.25;
    ...
    value: function _toggleZoom(clientX, clientY) {
      ...
      if (!this.zoomed) {
        this.zoomMouseMove = this.events.register(this.el, 'mousemove', ThemeUtils.throttle(function (event) {
          return _this2._positionZoom(event.clientX, event.clientY);
        }, 1000 / this.framerate));
        this.zoomMouseOut = this.events.register(this.el, 'mouseout', this._resetZoom);
        this.el.classList.add('product-image-zoomed');
        this.img.style.transform = "scale(".concat(this.scale, ")");
    ...
    value: function _resetZoom() {
      if (this.zoomMouseMove) {
        this.events.unregister(this.zoomMouseMove);
      }

      if (this.zoomMouseOut) {
        this.events.unregister(this.zoomMouseOut);
      }
...
var ProductGallery_ProductGallery = /*#__PURE__*/function () {
  function ProductGallery(el) {
    ...
    rimg_shopify_dist_index_es.watch(this.viewport);
    this.events = new EventHandler_default.a();
    ...
    if (settings.enable_image_zoom) {
      var images = this.viewport.querySelectorAll('[data-media-type="image"]');

      for (var i = 0; i < images.length; i++) {
        var image = images[i];
        var zoom = new ImageZoom_ImageZoom(image);
        this.media.push({
          type: 'image',
          figure: image,
  ...
  ProductGallery_createClass(ProductGallery, [{
    ...
    value: function _onModelLibraryLoad() {
      ...
      modelFigures.forEach(function (figure) {
        var el = figure.querySelector('model-viewer');
        var player = new Shopify.ModelViewerUI(el, {
          controls: controls
        });
    ...
    value: function _onVideoLibraryLoad() {
      ...
      var _loop = function _loop(i) {
        ...
        _this3.events.register(videoEl, 'play', function () {
    ...
    value: function _onExternalVideoLibraryLoad() {
      ...
      var _loop2 = function _loop2(i) {
        var videoEl = videoFigures[i];
        var player = new YT.Player(videoEl.querySelector('iframe'));
        var video = {
          type: 'video',
          figure: videoEl,
          player: player,
    ...
    value: function _onMediaSelect(media) {
      var viewInYourSpaceEl = this.el.querySelector('[data-shopify-xr]');

      if (viewInYourSpaceEl) {
        viewInYourSpaceEl.dataset.shopifyModel3dId = viewInYourSpaceEl.dataset.defaultModelId;
      }
      ...
      switch (media.type) {
        ...
        case 'model':
          ...
          viewInYourSpaceEl.dataset.shopifyModel3dId = this.selected.figure.dataset.media;
          break;
    ...
    value: function _selectMediaByIndex(index) {
      ...
      this.selected.figure.setAttribute('hidden', true);
      this.selected.figure = figure;
      this.selected.figure.dataset.productGallerySelected = true;
      this.selected.figure.removeAttribute('hidden');
...
function Product_updateCart() {
  window.Shopify.getCart(function (cart) {
    // Update cart text
    var cartAmountWrap = document.querySelector('[data-header-cart-count]');

    if (cartAmountWrap) {
      cartAmountWrap.innerHTML = "(".concat(cart.item_count, ")");
    }
  });
}
...
var Product_Product = /*#__PURE__*/function () {
  function Product(section) {
    ...
    this.productForm = this.el.querySelector('.product-details__form');
    this.addToCartButton = this.productForm.querySelector('[data-product-atc]');
    this.gallery = new ProductGallery_ProductGallery(this.el.querySelector('[data-product-gallery]'), {
      enable_image_zoom: this.data.gallery_image_zoom,
      enable_video_autoplay: this.data.gallery_video_autoplay,
      enable_video_looping: this.data.gallery_video_looping
    });
    this.productDetails = new ProductDetails_ProductDetails({
      el: this.el,
      product: this.product,
      settings: this.data,
      onVariantChange: function onVariantChange(_ref) {
        var variant = _ref.variant;
        return _this.gallery.selectMediaByVariant(variant);
      }
    });
    this.rtes = [];
    this.el.querySelectorAll('[data-rich-text-area]').forEach(function (el) {
    ...
    window.Shopify.onError = function (XMLHttpRequest) {
      return _this.handleErrors(XMLHttpRequest);
    };

    this.events.register(this.addToCartButton, 'click', function (event) {
  ...
  Product_createClass(Product, [{
    ...
    value: function addToCart(e) {
      ...
      if (this.data.enable_cart_redirection) {
        return;
      }

      e.preventDefault();
      var productMessageWrappers = this.el.querySelectorAll('.product-message');
      productMessageWrappers.forEach(function (el) {
      ...
      if (!this.addToCartButton.disabled) {
        ...
        this.addToCartButton.classList.add('product-details__add-to-cart-button--loading');
        var xhr = new XMLHttpRequest();
        var data = new FormData(this.productForm);
        ...
        xhr.onload = function () {
          ...
          setTimeout(function () {
            _this2.addToCartButton.classList.remove('product-details__add-to-cart-button--loading');

            var productMessageWrapper = document.createElement('div');
            productMessageWrapper.classList.add('product-message');
            productMessageWrapper.innerHTML = message;

            var productAddToCart = _this2.el.querySelector('.product-details__add-to-cart-area');

            productAddToCart.appendChild(productMessageWrapper);
        ...
        xhr.onerror = function () {
          _this2.handleErrors(xhr.statusText);
        };

        xhr.open('POST', window.Theme.routes.cart_add_url, true);
    ...
    value: function handleErrors(error) {
      ...
      setTimeout(function () {
        _this3.addToCartButton.classList.remove('product-details__add-to-cart-button--loading');

        if (error && error.responseJSON) {
          var productMessageWrapper = document.createElement('div');
          productMessageWrapper.classList.add('product-message');
          productMessageWrapper.innerHTML = error.responseJSON.description;

          var productAddToCart = _this3.el.querySelector('.product-details__add-to-cart-area');

          productAddToCart.appendChild(productMessageWrapper);
        }

        _this3.addingToCart = false;
      }, 1000);
    }
  }, {
    key: "unload",
...
var ProductQuickshop_ProductQuickshop = /*#__PURE__*/function () {
  ...
  ProductQuickshop_createClass(ProductQuickshop, [{
    ...
    value: function open() {
      ...
      this.modal = new Modal_Modal(modalCallbacks);
      this.modal.open("#".concat(id, " [data-product-quickshop-url=\"").concat(url, "\"]"), 'quickshop', trigger);
      this.isOpen = true;
      loader.then(function () {
        ...
        rimg_shopify_dist_index_es.watch(_this.el);
    ...
    value: function load() {
      ...
      this.loader = shopify_asyncview_dist_index_es.load(url, {
        ...
        makeIDsUnique(container.querySelectorAll('select[id], input[id]'), container.querySelectorAll('label[for]'));
        var targetContainer = _this3.isOpen ? _this3.modal.modalEl.querySelector('[data-modal-content]') : el;
        var spinner = targetContainer.querySelector('.quickshop__loading-state');
        spinner.parentNode.removeChild(spinner);
        _this3.el = targetContainer.appendChild(container);
        _this3.product = new Product_Product({
          el: container,
          data: data
        });
        ...
        console.warn('Unable to load quickshop', e);
        _this3.loader = null;
        return false;
      });
      return this.loader;
...
// CONCATENATED MODULE: ./source/scripts/components/ProductList.js
function ProductList_classCallCheck(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("Cannot call a class as a function"); } }

function ProductList_defineProperties(target, props) { for (var i = 0; i < props.length; i++) { var descriptor = props[i]; descriptor.enumerable = descriptor.enumerable || false; descriptor.configurable = true; if ("value" in descriptor) descriptor.writable = true; Object.defineProperty(target, descriptor.key, descriptor); } }

function ProductList_createClass(Constructor, protoProps, staticProps) { if (protoProps) ProductList_defineProperties(Constructor.prototype, protoProps); if (staticProps) ProductList_defineProperties(Constructor, staticProps); return Constructor; }





var isQuickShopTrigger = function isQuickShopTrigger(el) {
  return 'quickshopTrigger' in el.dataset;
};

var ProductList_ProductList = /*#__PURE__*/function () {
  function ProductList(_ref) {
    var el = _ref.el,
        id = _ref.id;

    ProductList_classCallCheck(this, ProductList);

    this.el = el;
    this.id = id;
    this._onRimgLoad = this._processImage.bind(this);

    this._load();
  }

  ProductList_createClass(ProductList, [{
    key: "reload",
    value: function reload() {
      // Only Share needs to be reloaded, everything else
      // operates on the overall element and will inherently
      // handle new content.
      this.share.unload();
      this.share = new Share_Share({
        el: this.el
      });
    }
  }, {
    key: "unload",
    value: function unload() {
      this.el.removeEventListener('rimg:load', this._onRimgLoad);
      this.share.unload();
      this.events.unregisterAll();
      this.quickshops.forEach(function (quickshop) {
        return quickshop.unload();
      });
    }
  }, {
    key: "_load",
    value: function _load() {
      ...
      this.el.addEventListener('rimg:load', this._onRimgLoad);
      this.share = new Share_Share({
        el: this.el
      });
      this.quickshops = new Map();
      this.events = new EventHandler_default.a();
      this.events.register(this.el, 'focusin', function (e) {
        return _this._loadQuickshop(e.target);
      });
      this.events.register(this.el, 'mouseover', function (e) {
        return _this._loadQuickshop(e.target);
      });
      this.events.register(this.el, 'click', function (e) {
        return _this._openQuickshop(e.target);
      });
    }
  }, {
    key: "_getQuickshop",
    value: function _getQuickshop(el) {
      var quickshop = this.quickshops.get(el);
      if (quickshop) return quickshop;
      quickshop = new ProductQuickshop_ProductQuickshop({
        el: el,
        id: this.id,
        url: el.dataset.productQuickshopUrl,
        hash: el.dataset.productQuickshopHash,
        // Ideally the trigger would be the actual quick shop trigger, but product cards
        // need a bunch of accessibility improvements before we can focus on that element
        trigger: el.closest('[data-product-item-id]').querySelector('[data-quickshop-trigger]')
      });
      this.quickshops.set(el, quickshop);
      return quickshop;
    }
  }, {
    key: "_loadQuickshop",
    value: function _loadQuickshop(target) {
      if (!isQuickShopTrigger(target)) return;
      var productItemEl = target.closest('[data-product-item-id]');
      var quickshopEl = productItemEl.querySelector('[data-product-quickshop]');

      this._getQuickshop(quickshopEl).load();
    }
  }, {
    key: "_openQuickshop",
    value: function _openQuickshop(target) {
      if (!isQuickShopTrigger(target)) return;
      var productItemEl = target.closest('[data-product-item-id]');
      var quickshopEl = productItemEl.querySelector('[data-product-quickshop]');

      this._getQuickshop(quickshopEl).open();
    }
  }, {
    key: "_processImage",
    value: function _processImage() {
      var unprocessedImages = this.el.querySelectorAll('.product-item-image:not(.processed)');

      for (var i = 0; i < unprocessedImages.length; i++) {
        var imageWrapper = unprocessedImages[i];
        var image = imageWrapper.querySelector('img');
        imageWrapper.classList.add('processed');

        if (image && image.getBoundingClientRect().height < 100) {
          var imageOverlay = imageWrapper.querySelector('.product-item-details-overlay');

          if (imageOverlay) {
            imageOverlay.classList.add('landscape');
          }
        }
      }
    }
  ...
  if (window.location.search.length) {
    ...
    for (var i = 0; i < aCouples.length; i++) {
      ...
      if (aKeyValue.length > 1 && aKeyValue[0] !== 'page') {
        window.Shopify.queryParams[decodeURIComponent(aKeyValue[0])] = decodeURIComponent(aKeyValue[1]);
      }
    }
  }
}

function updateSortFilter(e) {
  var search;
  var Sorting = {};
  Sorting.sort_by = document.querySelector('[data-sorting] select').value;

  if (e.target.closest('[data-sorting]').classList.contains('vendor-collection')) {
    ...
    window.location.search = "".concat(search, "&").concat(Object.keys(Sorting).map(function (k) {
      return "".concat(encodeURIComponent(k), "=").concat(encodeURIComponent(Sorting[k]));
    }).join('&'));
  } else {
    window.location.search = Object.keys(Sorting).map(function (k) {
      return "".concat(encodeURIComponent(k), "=").concat(encodeURIComponent(Sorting[k]));
    }).join('&');
  }
}
...
function updateTagFilter(e) {
  ...
  var shopifyParamsString = Object.keys(window.Shopify.queryParams).map(function (k) {
    return "".concat(encodeURIComponent(k), "=").concat(encodeURIComponent(window.Shopify.queryParams[k]));
  }).join('&');

  if (tag === 'all') {
    window.location.href = "".concat(window.Theme.routes.collections_url, "/").concat(collection, "?").concat(shopifyParamsString);
  } else {
    window.location.href = "".concat(window.Theme.routes.collections_url, "/").concat(collection, "/").concat(tag, "?").concat(shopifyParamsString);
  }
}
...
var Collection_Collection = /*#__PURE__*/function () {
  function Collection(section) {
    ...
    this.productList = new ProductList_ProductList({
      el: this.el,
      id: "shopify-section-".concat(section.id)
    });
    this.rtes = [];
    this.collectionTagFilterSelect = document.querySelector('[data-tag-filtering] select');
    this.collectionSortFilterSelect = document.querySelector('[data-sorting] select');
    this.el.querySelectorAll('[data-rich-text-area]').forEach(function (el) {
      return _this.rtes.push(new Rte_Rte({
        el: el
      }));
    ...
    if (section.type === 'static-collection') {
      setQueryParams();
  ...
  Collection_createClass(Collection, [{
    key: "_bindEvents",
    value: function _bindEvents() {
      if (this.collectionTagFilterSelect) {
        this.events.register(this.el.querySelector('[data-tag-filtering] select'), 'change', updateTagFilter);
      }

      if (this.collectionSortFilterSelect) {
        this.events.register(this.el.querySelector('[data-sorting] select'), 'change', updateSortFilter);
      }

      if (this.enableInfiniteScroll) {
        this.events.register(this.showMoreButton, 'click', this._infiniteScrollHandler);
      }
    }
  }, {
    key: "_initializeInfiniteScroll",
    value: function _initializeInfiniteScroll() {
      var _this2 = this;
      ...
      if (this.data.pagination_method.indexOf('infinite_scroll') > -1 && this.data.products_per_page < this.data.total_products && window.location.search.indexOf('page') === -1) {
        ...
        this.productsLoadedCount = this.el.querySelectorAll('[data-product-item-id]').length; // If we're infinite scrolling, we want to hide pagination
          ...
          var observer = new IntersectionObserver(function (entries) {
            entries.forEach(function (entry) {
              if (entry.intersectionRatio > 0) {
                _this2.scrollTargetVisible = true;

                _this2._infiniteScroll();
              } else {
                _this2.scrollTargetVisible = false;
              }
            });
          });
    ...
    value: function _infiniteScroll() {
      ...
      xhr.onload = function () {
        ...
        var products = response.querySelectorAll('[data-product-item-id]');
        var collectionGrid = document.querySelector('[data-collection-grid]');
        products.forEach(function (product) {
          collectionGrid.appendChild(product);
          _this3.productsLoadedCount++;

          _this3.productList.reload(); // Add a timeout to give rimg a little more time to load placeholder SVG
          // to prevent images from loading at the wrong size in Firefox


          setTimeout(function () {
            return rimg_shopify_dist_index_es.watch(product);
          }, 2);
        });

        if (_this3.productsLoadedCount === _this3.data.total_products) {
...
// CONCATENATED MODULE: ./source/scripts/sections/DynamicPromotions.js
function DynamicPromotions_classCallCheck(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("Cannot call a class as a function"); } }

function DynamicPromotions_defineProperties(target, props) { for (var i = 0; i < props.length; i++) { var descriptor = props[i]; descriptor.enumerable = descriptor.enumerable || false; descriptor.configurable = true; if ("value" in descriptor) descriptor.writable = true; Object.defineProperty(target, descriptor.key, descriptor); } }

function DynamicPromotions_createClass(Constructor, protoProps, staticProps) { if (protoProps) DynamicPromotions_defineProperties(Constructor.prototype, protoProps); if (staticProps) DynamicPromotions_defineProperties(Constructor, staticProps); return Constructor; }





var DynamicPromotions_DynamicPromotions = /*#__PURE__*/function () {
  function DynamicPromotions(section) {
    var _this = this;

    DynamicPromotions_classCallCheck(this, DynamicPromotions);

    this.el = section.el;
    this.blocks = this.el.querySelectorAll('[data-promotion-block]');
    this.events = new EventHandler_default.a();
    this.blocks.forEach(function (block) {
      rimg_shopify_dist_index_es.watch(block);

      _this.events.register(block, 'rimg:load', function () {
        return _this.checkBlockOverflow(block);
      });
    });
    this.events.register(window, 'resize', just_debounce_default()(function () {
      _this.blocks.forEach(function (block) {
        _this.checkBlockOverflow(block);
      });
    }, 100, true, true));
  }

  DynamicPromotions_createClass(DynamicPromotions, [{
    key: "checkBlockOverflow",
    value: function checkBlockOverflow(block) {
      var content = block.querySelector('[data-promotion-content]');
      var image = block.querySelector('[data-promotion-image] img');
      ...
      if (contentHeight > imageHeight) {
        block.style.setProperty('--promotion-block-height', "".concat(contentHeight, "px"));
        block.classList.add('promotion-block--content-overflow');
      } else {
        block.style.setProperty('--promotion-block-height', "".concat(imageHeight, "px"));
        block.classList.remove('promotion-block--content-overflow');
      }
    }
  }, {
...
var DynamicSlideshow_DynamicSlideshow = /*#__PURE__*/function () {
  function DynamicSlideshow(section) {
    ...
    if (this.carousel && this.slides.length > 1) {
      this.flickity = new js_default.a(this.carousel, {
        accessibility: false,
        // Flickity's inherent "accessibility" actually makes it worse
        adaptiveHeight: true,
        autoPlay: this.settings.enable_autoplay && !this.viewportIsMobile() ? this.settings.autoplay_interval * 1000 : false,
        cellAlign: 'left',
      ...
      this.flickity.on('change', function (index) {
        _this.carousel.style.setProperty('--slide-text-color', "".concat(_this.slides[index].dataset.textColor));
  ...
  DynamicSlideshow_createClass(DynamicSlideshow, [{
    ...
    value: function onSectionBlockSelect(block) {
      ...
      var slideIndex = block.el.dataset.slideshowSlideIndex;
...
var StaticAddresses_StaticAddresses = /*#__PURE__*/function () {
  function StaticAddresses(section) {
    ...
    var scripts = document.querySelector('[data-scripts]');
    script_default()([scripts.dataset.shopifyCommonUrl], function () {
      return script_default()([scripts.dataset.customerAreaUrl], function () {
        return _this._addAddressObservers();
      });
    });
  }
  ...
  StaticAddresses_createClass(StaticAddresses, [{
    key: "_addNewToggles",
    value: function _addNewToggles() {
      this.newToggles = this.el.querySelectorAll('[data-customer-toggle-new-form]');

      for (var i = 0; i < this.newToggles.length; i++) {
        this.newToggles[i].addEventListener('click', newToggleHandler);
      }
    }
  }, {
    key: "_addToggles",
    value: function _addToggles() {
      ...
      var toggleEls = this.el.querySelectorAll('[data-customer-toggle-form]');
      this.toggles = [];
      ...
      var _loop = function _loop(i) {
        ...
        _this2.toggles[i].handler = function (e) {
          e.preventDefault();
          window.Shopify.CustomerAddress.toggleForm(toggleEls[i].dataset.customerToggleForm);
          toggleEls[i].closest('[data-customer-address]').scrollIntoView();
        };

        toggleEls[i].addEventListener('click', _this2.toggles[i].handler);
      };
    ...
    key: "_addDeletes",
    value: function _addDeletes() {
      var _this3 = this;

      var deleteEls = this.el.querySelectorAll('[data-customer-delete-form]');
      this.deletes = [];
      ...
      var _loop2 = function _loop2(i) {
        ...
        _this3.deletes[i].handler = function (e) {
          e.preventDefault();
          window.Shopify.CustomerAddress.destroy(deleteEls[i].dataset.customerDeleteForm, _this3.data.message_confirm_delete);
        };

        deleteEls[i].addEventListener('click', _this3.deletes[i].handler);
      };

      for (var i = 0; i < deleteEls.length; i++) {
        _loop2(i);
      }
    }
  }, {
    key: "_getAddressIDs",
    value: function _getAddressIDs() {
      var addressIDEls = this.el.querySelectorAll('[data-address-id]');
      var addressIDs = [];
      addressIDEls.forEach(function (addressIDEl) {
        return addressIDs.push(addressIDEl.dataset.addressId);
      });
      return addressIDs;
    }
  }, {
    key: "_addAddressObservers",
    value: function _addAddressObservers() {
      var _this4 = this;

      this.addressIDs = this._getAddressIDs();
      this.addressObservers = []; // initialize new address form

      this.addressObservers.push(new window.Shopify.CountryProvinceSelector('address-country', 'address-province', {
        hideElement: 'address-province-container'
      })); // initialize edit address form(s)

      this.addressIDs.forEach(function (id) {
        _this4.addressObservers.push(new window.Shopify.CountryProvinceSelector("address-country-".concat(id), "address-province-".concat(id), {
    ...
    value: function onSectionUnload() {
      for (var i = 0; i < this.newToggles.length; i++) {
        this.newToggles[i].removeEventListener('click', newToggleHandler);
      }

      this.toggles.forEach(function (toggle) {
        return toggle.el.removeEventListener('click', toggle.handler);
      });
      this.deletes.forEach(function (del) {
        return del.el.removeEventListener('click', del.handler);
      });
      this.selects.forEach(function (select) {
        return select.unload();
      });
    }
  }]);
...
var StaticBlog = function StaticBlog(section) {
...
var StaticCart_StaticCart = /*#__PURE__*/function () {
  ...
  StaticCart_createClass(StaticCart, [{
    ...
    value: function _bindEvents() {
      ...
      var quantityInputs = this.el.querySelectorAll('[data-quantity-key]');
      var removeItemButtons = this.el.querySelectorAll('[data-remove]');
      quantityInputs.forEach(function (el) {
        return _this.events.register(el, 'input', function (event) {
          return _this._updateItemQuantity(event);
        });
      });
      removeItemButtons.forEach(function (el) {
        return _this.events.register(el, 'click', function (event) {
          return _this._updateItemQuantity(event);
        });
      });
    }
    ...
    value: function _updateItemQuantity(event) {
      ...
      if (currentTarget.hasAttribute('data-remove')) {
        quantity = 0;
        lineItemKey = event.currentTarget.dataset.lineItem;
      } else {
        ...
        lineItemKey = currentTarget.dataset.quantityKey;
      ...
      this.updateTimeout = setTimeout(function () {
        ...
        window.Shopify.changeItem(lineItemKey, quantity, function (cart) {
          // If another request is in progress, discard this update
          if (_this2.updateTimeout && _this2.updateTimeout !== thisTimeoutId) {
            return;
          }
          ...
          _this2.updateCart(cart, thisTimeoutId);
        });
      }, 300);
    }
  }, {
    ...
    value: function updateCart(cart, timeoutId) {
      ...
      shopify_asyncview_dist_index_es.load('/cart', {
        ...
        // If another request is in progress, discard this update
        if (_this3.updateTimeout !== timeoutId) {
          return;
        }

        _this3._unbindEvents(); // Inject new cart table contents


        var newTableContainer = document.createElement('div');
        newTableContainer.innerHTML = html.table;
        var cartTable = morphdom_esm(_this3.el.querySelector('[data-cart-table]'), newTableContainer.querySelector('table'), {
        ...
        rimg_shopify_dist_index_es.watch(cartTable);

        var cartDiscountContainer = _this3.el.querySelector('[data-cart-discount-container]');

        var totalPrice = _this3.el.querySelector('[data-total-price]');

        var cartItemCountWrap = document.querySelector('[data-header-cart-count]'); // Inject new cart level discounts

        var newDiscountContainer = document.createElement('div');
        newDiscountContainer.innerHTML = html.discounts;
        cartDiscountContainer.innerHTML = newDiscountContainer.querySelector('[data-cart-discount-container]').innerHTML; // Update total

        totalPrice.innerHTML = data.totalPrice;

        if (cartItemCountWrap) {
          cartItemCountWrap.innerText = "(".concat(data.itemCount, ")");
        }
...
// CONCATENATED MODULE: ./source/scripts/sections/StaticGiftCard.js
function StaticGiftCard_classCallCheck(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("Cannot call a class as a function"); } }
...
function initializeQRCode(element) {
  new window.QRCode(element, {
    text: element.dataset.giftCardQrCode,
    width: 120,
    height: 120
  });
}

var StaticGiftCard_StaticGiftCard = function StaticGiftCard(section) {
  StaticGiftCard_classCallCheck(this, StaticGiftCard);

  var scripts = document.querySelector('[data-scripts]');
  var cardCodeEl = section.el.querySelector('[data-gift-card-code]');
  var qrCodeEl = section.el.querySelector('[data-gift-card-qr-code]');

  if (cardCodeEl) {
    cardCodeEl.addEventListener('click', function () {
      return selectText(cardCodeEl);
    });
  }

  if (qrCodeEl) {
    script_default()([scripts.dataset.qrCodeJsUrl], function () {
      return initializeQRCode(qrCodeEl);
    });
  }
};
...
var NavDesktopParent_NavDesktopParent = /*#__PURE__*/function () {
  function NavDesktopParent(el, options) {
    ...
    for (var i = 0; i < children.length; i++) {
      if (children[i].dataset.navDesktopLink !== undefined) {
        this.link = children[i];
      } else if (children[i].dataset.navDesktopSubmenu !== undefined) {
    ...
    this.click = function (e) {
      ...
      if (e.target.dataset.navDesktopLink === undefined || _this._isOpen) {
        return;
      }
...
var NavDesktopMenu_NavDesktopMenu = /*#__PURE__*/function () {
  function NavDesktopMenu(_ref) {
    ...
    this.events = new EventHandler_default.a(); // Meganav, if any, that is fully open (not animating).
    ...
    for (var i = 0; i < this.children.length; i++) {
      ...
      if (child.dataset.navDesktopParent !== undefined) {
...
var NavMobileSubmenu_NavMobileSubmenu = /*#__PURE__*/function () {
  ...
  NavMobile_createClass(NavMobile, [{
    ...
    value: function close() {
      ...
      this.overlayAnimation.animateTo('closed').then(function () {
        _this2.mobileNav.dataset.open = false;

        if (_this2.submenu) {
          _this2.submenu.unload();
        }
      });
    }
  }, {
    ...
    value: function openSubmenu(e) {
      ...
      this.submenu = new NavMobileSubmenu_NavMobileSubmenu({
        submenu: submenu,
        trigger: submenuTrigger,
...
var StaticHeader_StaticHeader = /*#__PURE__*/function () {
  function StaticHeader(section) {
    ...
    this.navDesktopMenu = new NavDesktopMenu_NavDesktopMenu(this.navigation);
    ...
    document.body.style.setProperty('--scroll-away-opacity', 1); // This is the logo height plus the announcement bar height to have the
    ...
    var barHeight = this.announcementBar ? this.announcementBar.getBoundingClientRect().height : 0;
    var fadeHeight = Math.max(this.logo.getBoundingClientRect().height + barHeight - 10, 100);
    this.events.register(window, 'scroll', function () {
      var opacity = Math.max(1 - window.scrollY / fadeHeight, 0);
      document.body.style.setProperty('--scroll-away-opacity', opacity);

      if (_this.sticky) {
        var searchBottom = _this.searchInput.getBoundingClientRect().bottom;

        var navTop = _this.navigation.getBoundingClientRect().top;

        if (searchBottom > navTop) {
          _this.headerControls.classList.add('header-controls--search-overlap');

          _this.header.classList.add('site-header--search-overlap');
        } else {
          _this.headerControls.classList.remove('header-controls--search-overlap');

          _this.header.classList.remove('site-header--search-overlap');
        }
      }
    });
    this.events.register(this.searchInput, 'focusin', function () {
      _this.headerControls.classList.add('header-controls--search-active');

      _this.header.classList.add('site-header--search-active');
    });
    this.events.register(this.searchInput, 'focusout', function () {
      _this.headerControls.classList.remove('header-controls--search-active');

      _this.headerControls.classList.remove('header-controls--search-overlap');

      _this.header.classList.remove('site-header--search-active');
...
var StaticPassword_StaticPassword = /*#__PURE__*/function () {
  function StaticPassword(_ref) {
    var _this = this;
    ...
    this.el = el;
    this.modal = new Modal_Modal();
    this.events.register(document.querySelector('[data-open-modal-button]'), 'click', function () {
  ...
  StaticPassword_createClass(StaticPassword, [{
    ...
    value: function openModal() {
      this.modal.open('.password-page-modal-content', 'password');
    }
  }, {
    key: "onSectionUnload",
    value: function onSectionUnload() {
...
var StaticPage_StaticPage = /*#__PURE__*/function () {
...
var StaticProductRecommendations_StaticProductRecommendations = /*#__PURE__*/function () {
  function StaticProductRecommendations(section) {
    ...
    var recommendationsEl = document.querySelector('[data-product-recommendations]');
  ...
  StaticProductRecommendations_createClass(StaticProductRecommendations, [{
    ...
    value: function _loadRecommendations(container, productId, limit) {
      ...
      var url = "".concat(window.Theme.routes.product_recommendations_url, "?section_id=").concat(this.sectionId, "&limit=").concat(limit, "&product_id=").concat(productId);
      shopify_asyncview_dist_index_es.load(url, {
        ...
        _this.productList = new ProductList_ProductList({
          el: container,
          id: "shopify-section-".concat(_this.sectionId)
        });
...
// CONCATENATED MODULE: ./source/scripts/Editions.js
...
var Editions_scripts = document.querySelector('[data-scripts]');
script_default()([Editions_scripts.dataset.shopifyApiUrl], function () {
  ...
  sections.register('dynamic-featured-collection', function (section) {
    return new Collection_Collection(section);
  });
  sections.register('dynamic-featured-product', function (section) {
    return new Product_Product(section);
  });
  sections.register('dynamic-promotions', function (section) {
    return new DynamicPromotions_DynamicPromotions(section);
  });
  ...
  sections.register('pxs-video', function (section) {
    return new dist_index_es(section);
  });
  sections.register('static-addresses', function (section) {
    return new StaticAddresses_StaticAddresses(section);
  });
  sections.register('static-article', function (section) {
    return new Share_Share(section);
  });
  ...
  sections.register('static-contact-page', function (section) {
  ...
  sections.register('static-gift-card', function (section) {
    return new StaticGiftCard_StaticGiftCard(section);
  });
  ...
  sections.register('static-search-results', function (section) {
    return new ProductList_ProductList(section);
  });
  FlickityTouchFix();
});
...
/***/ })
/******/ ]);
//# sourceMappingURL=editions.js.map?1612973695341